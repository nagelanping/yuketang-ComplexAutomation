...
- 批量章节自动展开，逐个处理内部视频、音频、图文、作业；讨论区（`taolun` / `forum`）读教师要求后由模型生成回复并自动发表（关闭 AI 自动作答时改为跳过）。
- PPT/课件自动翻页与播放。
- 作业使用浏览器内截图能力截取题面，再通过 OpenAI-compatible API 调用多模态模型作答。
- 讨论区走纯文本：教师正文直接作为文本请求发给模型，不用截图（该页文本没有字体混淆）。
- 支持选择题、多选题、判断题、填空题。
...
- `暂停` / `继续`：暂停或恢复当前循环。注意它只挂起等待，**不会取消已经在途的 AI 请求**——那个请求返回后流程仍会继续往下走（选答案、点提交）。要彻底停住用 `终止`。
- `终止`：硬停。立刻取消在途的 AI 请求、清掉续跑标记，各循环入口随即退出，且不再接受「开始」。恢复方式只有刷新页面。
