...
- 批量章节自动展开，逐个处理内部视频、音频、图文、作业；讨论区（`taolun` / `forum`）自动回复尚未实现，就地跳过。
- PPT/课件自动翻页与播放。
...
- 雨课堂字体反混淆常开（截图答题依赖）：题目文本被混淆字体做了字形置换，脚本还原后截图与复制才是真实文字。
...
- `模型设置`：AI 接口配置与功能开关，含接口地址、API Key、模型名、接口格式、鉴权方式、最大 Token、thinking、流式传输、自动作答。
- `清除失败记录`：清空本会话记住的"反复推不动"章节计数与续跑标记。仅在某些项被安全阀跳过、想重新尝试时使用。
- `暂停` / `继续`：暂停或恢复当前循环。

## 纯刷视频模式

不开启 AI 自动作答时，脚本进入纯刷视频模式：作业类项被直接无视，只刷视频、音频、课件。需要自动答题时再在 `模型设置` 里开启。
